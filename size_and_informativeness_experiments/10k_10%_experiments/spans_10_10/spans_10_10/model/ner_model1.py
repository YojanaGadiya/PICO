...
from .data_utils1 import minibatches, pad_sequences, get_chunks, NONE, idx_word
...
class NERModel(BaseModel):
    ...
    def run_evaluate(self, test):
        ...
        for tag, counts in stats.items():
          tag_p = counts['n_correct'] / counts['n_pred'] if counts['n_correct'] > 0 else 0
          tag_r = counts['n_correct'] / counts['n_true'] if counts['n_correct'] > 0 else 0
          results['p'][tag] = tag_p
          results['r'][tag] = tag_r
          results['f1'][tag] = 2 * tag_p * tag_r / (tag_p + tag_r) if counts['n_correct'] > 0 else 0
